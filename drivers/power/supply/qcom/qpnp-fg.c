...
static void dump_debug(struct work_struct *work)
{
	...
	u8 *buffer, rt_sts;
	char str[16];
	struct fg_chip *chip = container_of(work, struct fg_chip, dump_sram);
	...
	if (rc)
		...
		pos += sprintf(chip->debug_dump + pos,
				"soc-rt-sts: 0x%0x    ", rt_sts);
	...
	for (i = 0; i < SRAM_DUMP_LEN; i += 4) {
		...
		pos += sprintf((chip->debug_dump + pos),
				"addr:%03x:%s   ", SRAM_DUMP_START + i, str);
	}
